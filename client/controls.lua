...
local function forAllPads(func)
    for _, value in pairs(ControlPads) do
        if ControlPads.ALL ~= value then
            func(value)
        end
    end
end
...
function Controls.disableThisFrame(control, pad) 
    if nil == pad then
        pad = ControlPads.ALL
    end

    if ControlPads.ALL == pad then
        forAllPads(function(_pad)
            Controls.disableThisFrame(control, _pad)
        end)

        return
    end

    DisableControlAction(pad, control, true)
end


function Controls.isEnabled(control, pad)
    if nil == pad or ControlPads.ALL == pad then
        pad = ControlPads.PLAYER_CONTROL
    end
    
    return IsControlEnabled(pad, control)
end


function Controls.isJustPressed(control, pad, disabled)
    if nil == disabled and "boolean" == type(pad) then
        disabled = pad
        pad = ControlPads.PLAYER_CONTROL
    end
    ...
    if true == disabled then
        return IsDisabledControlJustPressed(pad, control)
    end
    
    return IsControlJustPressed(pad, control)
end


function Controls.isJustReleased(control, pad, disabled)
    if nil == disabled and "boolean" == type(pad) then
        disabled = pad
        pad = ControlPads.PLAYER_CONTROL
    end
    ...
    if true == disabled then
        return IsDisabledControlJustReleased(pad, control)
    end
    
    return IsControlJustReleased(pad, control)
end


function Controls.isPressed(control, pad, disabled)
    if nil == disabled and "boolean" == type(pad) then
        disabled = pad
        pad = ControlPads.PLAYER_CONTROL
    end
    ...
    if true == disabled then
        return IsDisabledControlPressed(pad, control)
    end
    
    return IsControlPressed(pad, control)
end


function Controls.isReleased(control, pad, disabled)
    if nil == disabled and "boolean" == type(pad) then
        disabled = pad
        pad = ControlPads.PLAYER_CONTROL
    end
    ...
    if true == disabled then
        return IsDisabledControlReleased(pad, control)
    end
    
    return IsControlReleased(pad, control)
end


function Controls.getNormal(control, pad, unbound)
    if nil == unbound and "boolean" == type(pad) then
        unbound = pad
        pad = ControlPads.PLAYER_CONTROL
    end
    ...
    if true == unbound then
        return GetControlUnboundNormal(pad, control)
    end
    
    return GetControlNormal(pad, control)
end


function Controls.getValue(control, pad)
    if nil == pad or ControlPads.ALL == pad then
        pad = ControlPads.PLAYER_CONTROL
    end

    return GetControlValue(pad, control)
end


function Controls.getDisabledNormal(control, pad, unbound)
    if nil == unbound and "boolean" == type(pad) then
        unbound = pad
        pad = ControlPads.PLAYER_CONTROL
    end
    ...
    if true == unbound then
        return GetDisabledControlUnboundNormal(pad, control)
    end
    
    return GetDisabledControlNormal(pad, control)
end


function Controls.setNormal(control, pad, amount)
    if nil == amount and "number" == type(pad) then
        amount = pad
        pad = ControlPads.PLAYER_CONTROL
    end
    ...
    return SetControlNormal(pad, control, amount)
end


function Controls.shakePad(pad, duration, freq)
    if nil == freq then
        freq = duration
        duration = pad
        pad = ControlPads.ALL
    ...
    if ControlPads.ALL == pad then
        forAllPads(function(_pad)
            Controls.shakePad(_pad, duration, freq)
        end)

        return
    end

    SetPadShake(pad, duration, freq)
end


function Controls.stopShakingPad(pad)
    if nil == pad then
        pad = ControlPads.ALL
    end

    if ControlPads.ALL == pad then
        forAllPads(Controls.stopShakingPad)
        return
    end
